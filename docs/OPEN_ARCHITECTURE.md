...
**책임:** 단타 매수 신호 스캔 및 분석 (7개 체크리스트)

```python
# 7개 체크리스트 점수 (총 105점)
{
    "volume_score": 15,      # 거래량 폭증
    "momentum_score": 15,    # 모멘텀 돌파
    "box_score": 15,         # 박스권 탈출
    "ma5_score": 15,         # 5일선 위
    "institution_score": 15, # 기관 매수
    "oversold_score": 15,    # 낙폭 과대
    "sector_score": 15,      # 섹터 모멘텀
}
# 총점: 0-105점 → 등급: S(90+), A(75-89), B(60-74), C(60미만)
```

**주요 엔드포인트:**
- `GET /api/daytrading/scan` - 단타 시그널 스캔
- `GET /api/daytrading/signals` - 활성 단타 시그널
- `GET /api/daytrading/{ticker}` - 종목별 단타 분석
- `GET /api/daytrading/stats` - 스캔 통계
