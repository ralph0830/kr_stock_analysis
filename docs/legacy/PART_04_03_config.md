# PART 04-3: SignalConfig (시그널 설정)

> **파일**: `engine/config.py`
> **용도**: 종가베팅 V2 시그널 생성기 설정 (dataclass)
> **의존**: 없음
...
`SignalConfig`는 종가베팅 V2 시스템의 모든 설정을 포함하는 dataclass입니다.
...
| 등급 | 최소 거래대금 | 등락률 범위 | 최소 점수 | R 배수 | 포지션 |
|-----|-------------|-----------|---------|--------|--------|
| **S** | 1조원 | +10%~+20% | 10점+ | 1.5R | 15% |
| **A** | 5천억원 | +8%~+15% | 8점+ | 1.0R | 10% |
| **B** | 1천억원 | +5%~+12% | 6점+ | 0.5R | 5% |
| **C** | 500억원 | +5%~+29.9% | - | - | 매매안함 |
...
# 커스텀 설정
config = SignalConfig(
    min_trading_value=100_000_000_000,  # 1000억 이상
    max_positions=3,                      # 최대 3개 포지션
    r_ratio=0.01,                         # R 비율 1%
)

# 프리셋 (선택적 구현)
conservative = SignalConfig.default().conservative()
aggressive = SignalConfig.default().aggressive()
```
...
def validate(self) -> bool:
    ...
    if self.min_trading_value < 0:
        return False
    if self.min_change_pct < 0 or self.max_change_pct > 30:
        return False
    if self.r_ratio <= 0 or self.r_ratio > 0.1:
        return False
    return True
```
