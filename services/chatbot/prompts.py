...
- AI 감성 분석 결과
- **뉴스 링크 (news_urls)**: 실제 네이버 뉴스 기사 URL

## ⚠️ 뉴스 처리 규칙 (매우 중요!)
1. **제공된 news_urls만 사용**: 시스템이 제공한 news_urls 리스트에 있는 뉴스만 참고하세요
2. **절대 가짜 뉴스 생성 금지**: news_urls에 없는 뉴스는 절대로 만들어내지 마세요
3. **LLM 지식 우선 금지**: 본인의 학습 데이터에서 뉴스를 가져오지 말고, 반드시 제공된 news_urls를 사용하세요
4. **링크 형식 준수**: 뉴스 링크는 반드시 `[제목](URL)` 형식으로 표시하세요
5. **news_urls가 비어있을 때**: 실제 뉴스가 없으면 "최근 수집된 뉴스가 없습니다"라고만 하고 절대 가짜 뉴스를 만들지 마세요
...
def build_rag_prompt(
    ...
    if context.get("news"):
        parts.append("\n## 실시간 뉴스 (반드시 이 뉴스만 사용하세요)")
        for news in context["news"][:3]:
            summary = news.get('summary', '분석 내용')[:200]
            parts.append(f"요약: {summary}")

            # 뉴스 링크 추가 - 더 명확하게 표시
            news_urls = news.get('news_urls', [])
            if news_urls:
                parts.append(f"\n📰 **실제 뉴스 링크 ({len(news_urls)}건): **")
                for i, url_info in enumerate(news_urls[:5], 1):  # 최대 5개 링크
                ...
                # 명확한 지시
                parts.append(f"\n⚠️ **위 {len(news_urls[:5])}개의 뉴스 링크만 사용하세요. 다른 뉴스를 만들어내지 마세요.**")
            else:
                parts.append("\n⚠️ 수집된 뉴스가 없습니다. 뉴스를 생성하지 마세요.")
